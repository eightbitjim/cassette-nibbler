...
public class TRS80FileStateMachine implements FileStreamProvider, PulseStreamConsumer {
    ...
    private transient TRS80TapeBlock currentBlock;
    private transient TRS80ByteFrame currentByte;
    private transient TRS80TapeFile currentFile;
    ...
    public TRS80FileStateMachine() {
        currentByte = new TRS80ByteFrame();
        reset();
    }
    ...
    private void createNewBlock() {
        currentBlock = new TRS80TapeBlock(currentFile.nextBlockType());
    }

    private void processPulseInBlock() {
        int result = currentByte.addPulse(currentPulse, currentTimeIndex);
        switch (result) {
            case TRS80ByteFrame.MORE_BITS_NEEDED:
                break;
            case TRS80ByteFrame.ERROR:
